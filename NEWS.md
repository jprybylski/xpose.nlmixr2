...
* `xpose_data_nlmixr2()` no longer crashes with "undefined columns selected" for FOCEi fits where one or more etas are fixed to zero variance (`eta ~ 0`), which causes the omega matrix to include more etas than the eta table (#10).
* The default `gg_theme` argument is now `theme_readable` (the function itself) rather than `theme_readable()` (a pre-evaluated theme object), matching the convention used in the parent xpose package (#9).
